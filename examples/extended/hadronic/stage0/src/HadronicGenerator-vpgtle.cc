...
HadronicGenerator::HadronicGenerator( const G4String physicsCase ) :
  fPhysicsCase( physicsCase ), fLastHadronicProcess( nullptr ), fPartTable( nullptr )
{
  ...
  if ( fPhysicsCase!= "QGSP_BIC_HP" ) {
    G4cerr << "ERROR: Not supported final-state hadronic inelastic physics case !"
           ...
           << "\t - \"Physics-list proxies\" : QGSP_BIC_HP"
           << G4endl;
  }
  ...
  G4VCrossSectionDataSet* theNeutronHPFissionXS = new G4NeutronHPFissionData;
  theNeutronHPFissionXS->BuildPhysicsTable( *(G4Neutron::Definition()) );

  G4VCrossSectionDataSet* theNeutronHPXSdata = new G4NeutronHPCaptureData;
  theNeutronHPXSdata->BuildPhysicsTable( *(G4Neutron::Definition()) );
  ...
  // interaction that are considered as relevant are the capture and fission (+ inelastic HP process neutron -nucleus)
  //Prompt-gamma timing with a track-length estimator in proton therapy, JM Létang et al, 2024

  G4HadronicProcess* theNeutronHPInelasticProcess =
    new G4HadronInelasticProcess( "neutronInelasticHP", G4Neutron::Definition() );
  fProcessMap.insert( ProcessPair( G4Neutron::Definition(), theNeutronHPInelasticProcess ) );

  G4HadronicProcess* theNeutronCaptureProcess =
    new G4HadronInelasticProcess("neutronCaptureHP", G4Neutron::Definition());
  fProcessMap.insert( ProcessPair( G4Neutron::Definition(), theNeutronCaptureProcess ) );

  G4HadronicProcess* theNeutronHPFissionProcess =
    new G4HadronInelasticProcess("neutronFissionHP", G4Neutron::Definition());
  fProcessMap.insert( ProcessPair( G4Neutron::Definition(), theNeutronHPFissionProcess ) );
  ...
  // in the physics list QGSP_BIC, BIC is used only for nucleons
...
G4int HadronicGenerator::GenerateInteraction(G4ParticleDefinition* projectileDefinition, G4Material* targetMaterial,
					     TH2D* TH2D_EpEpg, TH1D* TH1D_SigmaInelastic, TH2D* TH2D_GammaZ, TH1D* TH1D_NrPG) {
    ...
    G4int numCollisions = 1e2;  //***LOOKHERE***  NUMBER OF COLLISIONS
    G4double rnd1(0);
    G4double protonEnergy(0), GAMMAZPerRho(0), kappaPerRho(0), kappaPerRhoMmCollisions(0);
    G4int Ngamma(0);
    ...
    // G4cout << energyStart << " " << energyEnd << " "<< nbBins << " " << energyIncrement << G4endl;
    ...
    for (G4int f = 1; f <= nbBins; f++) {
      ...
      G4DynamicParticle* dParticle = new G4DynamicParticle(projectileDefinition, projectileDirection, energyEnd);
	...
	projectileEnergy = (TH1D_SigmaInelastic->GetBinCenter(f) + (G4UniformRand()-0.5)*energyIncrement) * CLHEP::MeV;
	...
	if (theProcess != nullptr) {
	  // G4cout << "\n\nTheProcess is " << theProcess->GetProcessName() << G4endl; 
	...
	// The assumption in the prompt-gamma TLE is to use the defaut distance unit, ie mm ($G4_DIR/src/source/externals/clhep/include/CLHEP/Units/SystemOfUnits.h)
	...
	// delete dynamicProjectile;    
      ...
      dParticle->SetKineticEnergy(TH1D_SigmaInelastic->GetBinCenter(f) * CLHEP::MeV);
      CrossSection = GetCrossSection(dParticle, targetElement, targetMaterial);
      kappaPerRho = (CrossSection * Avogadro) / molarMass;
...
G4double HadronicGenerator::GetCrossSection(const G4DynamicParticle* part, const G4Element* elm, const G4Material* targetMaterial) {
    ...
    CrossSection = hadProcess->GetCrossSectionDataStore()->GetCrossSection(part, elm, targetMaterial);
